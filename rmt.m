    ...
    case 'run_path_planning'
        ...
        switch mission_task
            case 2
                ...
                tic;
                [run_Tg,run_B,run_P,path_Tg,path_B,cost_optim] = rmt_find_accepted_run_multicost(Pg,'prob','move');  %solution in Pg and projection to Tg and B
                delete(data.hwait);
                message2 = sprintf('\nTime for finding accepted run: %g secs', toc);
                message = sprintf('%s%s', message, message2);
                uiwait(msgbox(message2,'Robot Motion Toolbox','modal'));
                [R_runs,R_paths,R_trajs,active_robots] = rmt_robot_trajectory_team(data.T,Tg,run_Tg,path_Tg);  %each robot starts from centroid of its initial cell; R_trajs is a cell array,
                %we clean the workspace figure
                ...
                switch planning_approach
                    case 1%cell decomposition
                        ...
                        if (get(findobj(gcf,'Tag','waypoints'),'Value') == 1) %middle points
                            ...
                            data.trajectory = traj;
                            message = sprintf('Travelled distance via middle points: %g.\n',travel_dist);
                            uiwait(msgbox(message,'Robot Motion Toolbox','modal'));
                        elseif (get(findobj(gcf,'Tag','waypoints'),'Value') == 2)  %norm 1
                            ...
                            message = sprintf('Travelled distance via Norm-1: %g.\n',dist_norm_one);
                            uiwait(msgbox(message,'Robot Motion Toolbox','modal'));
                        elseif (get(findobj(gcf,'Tag','waypoints'),'Value') == 3)  %norm 2
                            ...
                            message = sprintf('Travelled distance via Norm-2: %g.\n',dist_norm_2_sq);
                            uiwait(msgbox(message,'Robot Motion Toolbox','modal'));
    ...
    case 'ltl_pn'
        ...
        for i = 1 : 2*data.intermediateMarkings
            ...
            if (i/2 == round(i/2))
                ...
                input_place = input_place(find(input_place>nplaces_orig+length(data.Tr.props)))-nplaces_orig-length(data.Tr.props); %take only the place of the buchi
                output_place = find(Post(:,trans_buchi+ntrans_orig));
                output_place = output_place(find(output_place>nplaces_orig+length(data.Tr.props)))-nplaces_orig-length(data.Tr.props);
