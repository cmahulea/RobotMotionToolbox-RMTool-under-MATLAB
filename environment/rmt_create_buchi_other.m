...
if (strcmpi(temp,'PCWIN') || strcmpi(temp,'PCWIN64'))
    [s,r]=dos(['.\aux_toolboxes\ltl2ba\ltl2ba.exe -d -f "' formula '"']); %sintax for calling ltl2ba.exe (located in subdir .\ltl2ba); use full description result (-d)
    %example of run: [s,r]=dos('ltl2ba -d -f "<> p1"'); %LTL 2 Buchi for our formula
elseif strcmpi(temp,'GLNXA64')
    [s,r]=unix(['/home/cristian/Dropbox/RobotMotionToolbox/aux_toolboxes/ltl2ba/ltl2bal -d -f "' formula '"']);
    %[s,r]=unix(['/home/luis/workspace/RobotMotionToolbox/aux_toolboxes/ltl2ba/ltl2bal -d -f "' formula '"']);
    %[s,r]=unix(['/home/luis/Dropbox/RobotMotionToolbox/aux_toolboxes/ltl2ba/ltl2bal -d -f "' formula '"']);
elseif strcmpi(temp,'MACI64')
    [s,r]=unix(['/Users/cristi/Dropbox/software/RobotMotion/CurrentVersion/aux_toolboxes/ltl2ba/ltl2ba -d -f "' formula '"']);
end
