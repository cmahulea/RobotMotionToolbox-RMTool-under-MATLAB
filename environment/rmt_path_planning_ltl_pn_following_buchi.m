...
for p_B=1:length(paths_B)
    ...
    for i=1:(length(path_B)-1)  %find traj in PN s.t. Buchi goes from state path_B(i) to path_B(i+1), without getting another observation that leaves path_B(i) to other state
        ...
        [feasible_sol , marking_final, Rob_positions_final, Rob_places_temp, Rob_trans_temp, ...
            ...
            data.T.props, Obs, 'final', obs_fin, obs_feasible, data.intermediateMarkings, solver, xmin, ...
        ...
        %*** 3) use intermediate markings to keep only feasible observations for staying on path path_B(i) -> path_B(i+1) in Buchiitions that should be avoided along reached markings
        k=max([min_k1+N_r, min_k2+N_r, min_k]);  %value of k chosen based on ILP1,2 and predefined value
        fprintf('\n\t Solving ILP 3, k= %g, with solver %s ... ',k,solver);
        switch solver
            case 'glpk'
                [cost, matrix_A, vector_b, lb, ub, ctype, vartype, sense] = rmt_constraints_PN_obs(Pre, Post, PN_marking, ...
        ...
        %test solution feasibility (non-spurious sigma and generated observables do not leave current transition in Buchi)
        [feasible_sol , marking_final, Rob_positions_final, Rob_places_temp, Rob_trans_temp, Rob_synchronize_temp, ~] = test_solution_feasibility(Pre, Post, PN_marking, T.props, Obs, 'intermediate', obs_fin, obs_feasible, k, solver, xmin, status, Rob_positions);
        
        if feasible_sol==1 %ILP3 gives good result, go to next transition in Buchi
            Rob_positions = Rob_positions_final;
            PN_marking = marking_final;
            for j=1:N_r
                Rob_synchronize{j}=[Rob_synchronize{j} , Rob_synchronize_temp{j} + length(Rob_places{j})-1]; %when to synchronize (number of visited place)
