...
function [A,b,Aeq,beq,cost] = rmt_construct_constraints_final(Pre,Post,m0, props, formula, obstacles)

%the variables are: [m_1 sigma_1 m_2 sigma_2 .... m_r+1 sigma_r+1 m_r+2(==mf) sigma_r+2 no_of_binary_variables_traj ]

nplaces = size(Pre,1); %number of places
ntrans = size(Pre,2); % number of transitions
N = sum(m0); % number of robots

Aeq = [eye(nplaces) -(Post-Pre) zeros(nplaces,(nplaces+ntrans)*(N+1))];
beq = m0;
for i = 2 : N+2
    %add the state equation: m_i = m_{i-1} + (Post-Pre)*sigma_i
    Aeq = [Aeq;zeros(nplaces,(nplaces+ntrans)*(i-2)) -eye(nplaces) zeros(nplaces,ntrans) eye(nplaces) -(Post-Pre) zeros(nplaces,(nplaces+ntrans)*(N+2-i))];
...
for i = 1 : length(props)
    ...
    vii(props{i} + (nplaces+ntrans)*(N+1))=1;
...
%%%add a constraint to fire only one transition per robot for the final
%%%movement

A = [A; zeros(nplaces,(nplaces+ntrans)*(N)) -eye(nplaces) zeros(nplaces,ntrans+nplaces) Pre zeros(nplaces,length(props))];
b = [b; zeros(nplaces,1)];

%%%%%%%%%%% cost function
cost = zeros(1,size(Aeq,2));
for i = 1 : N+2
    cost((i-1)*(nplaces+ntrans)+nplaces+1:(i-1)*(nplaces+ntrans)+ntrans+nplaces) = i;
end
%parse formula and convert to linear constraints/restrictions
[~,~,~,A_r,b_r] = rmt_formula2constraints(formula, A,b,length(props));
A_r(:,1:length(props)) = [];
for i = size(A_r,1):-1:1  %remove the empty lines that corresponds to formula on the trajectory
    if isempty(find(A_r(i,:), 1))
        A_r(i,:) = [];
        b_r(i) = [];
    end
end
...
if ~isempty(A_r)
    A = [A; zeros(size(A_r,1),(N+2)*(nplaces+ntrans)) A_r];
    b = [b;b_r];
end
