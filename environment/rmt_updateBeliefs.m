    ...
    case 1 %Each robot acts independently
        for i=1:N_r
            for x=1:length(T.Q)
                ...
                diff = abs(Robots{i}.Beliefs{x}.Prob(length(T.props)+1) - max(Robots{i}.Beliefs{x}.Prob));
                if (diff < 0.01)%if more regions have the same probability including the empty observation (free space), consider the free space
