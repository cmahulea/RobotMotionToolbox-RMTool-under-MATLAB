...
function [short_traj , traj_length] = rmt_optimize_traj_norm_inf(com_F,path_cells,start_p,goal_p,safe_dist)
%% formulate LP for minimizing sum of norm inf (max. abs. value) from all trajectory segments
...
%2*n_lam+1 variables in LP
% A=zeros(4*n_lam+4 , 2*n_lam+1);
% b=zeros(4*n_lam+4,1);
A=[];
b=[];
cost_f=[zeros(n_lam,1) ; ones(n_lam+1,1)];  %linear term of cost funct.
if n_lam==0 %particular cases
    short_traj=[start_p , goal_p];  %in the same cell
    return;
elseif n_lam==1
    ax1=zeros(1,2*n_lam+1); %first segment
    ax1(1)= (X{1}(2)-X{1}(1));
    ax2=-ax1;
    ax1(n_lam+1)=-1;
    ax2(n_lam+1)=-1;
    bx1= start_p(1)-X{1}(1);
    bx2= -bx1;
    ay1=zeros(1,2*n_lam+1); %current line in A (for y)
    ay1(1)= (Y{1}(2)-Y{1}(1));
    ay2=-ay1;
    ay1(n_lam+1)=-1;    %same bound var for norm inf
    ay2(n_lam+1)=-1;
    by1= start_p(2)-Y{1}(1);
    by2= -by1;
    A=[A; ax1;ax2;ay1;ay2];
    ...
    ax1=zeros(1,2*n_lam+1); %last segment
    ...
    bx2= -bx1;
    ay1=zeros(1,2*n_lam+1); %current line in A
    ...
    ax1=zeros(1,2*n_lam+1); %first restrictions
    ax1(1)= (X{1}(2)-X{1}(1));
    ax2=-ax1;
    ax1(n_lam+1)=-1;
    ax2(n_lam+1)=-1;
    ...
    for i=1:(n_lam-1)
        ax1=zeros(1,2*n_lam+1); %current line in A (for X)
        ax1(i)= -(X{i}(2)-X{i}(1));
    ...
    ax1=zeros(1,2*n_lam+1); %constraints for last segment
    ax1(n_lam)= -(X{n_lam}(2)-X{n_lam}(1));
    ax2=-ax1;
    ax1(2*n_lam+1)=-1;
    ax2(2*n_lam+1)=-1;
...
low_bound=[zeros(n_lam,1)+epsilon ; zeros(n_lam+1,1)];  %zero lower limit for abs. values (c's)
up_bound=ones(n_lam,1)-epsilon;
A=[A; [eye(n_lam) , zeros(n_lam,n_lam+1)]]; %upperbound for lamb. in A, because no upper for c
b=[b; up_bound];
...
try
    ...
    if exitflag~=1
        fprintf('\nERROR when solving LP for infinity norm.\n');
    end
catch
